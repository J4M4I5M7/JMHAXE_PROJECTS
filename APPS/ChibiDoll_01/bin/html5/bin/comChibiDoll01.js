...
ApplicationMain.embed = $hx_exports.openfl.embed = function(elementName,width,height,background) {
	...
	id = "img/PreviewImage150x150.png";
...
Main.prototype = $extend(openfl.display.Sprite.prototype,{
	...
	,init: function() {
		...
		this.inited = true;
		this._headBody = this.makeBody();
		this._eyes = this.makeEyes();
		this._mouth = this.MakeMouth();
		this._eyes.addAnim("blink_eyes",[2,3,4],false);
		this._eyes.addAnim("idle_eyes",[0,1],true);
		this._eyes.setDefaultAnim("idle_eyes");
		this._eyes.playAnim("idle_eyes");
		this._chibi = new jmSubLibs.decoupledLIB.doll.ChibiSpace(100,160);
		this._chibi.add(this._headBody);
		this._chibi.add(this._eyes);
		this._chibi.add(this._mouth);
		this._chibi.heldFrames = 8;
		this._chibi.setClickFN($bind(this,this.onChibiClickFN));
		this.addChild(this._chibi);
	}
	,onChibiClickFN: function() {
		this._eyes.playAnim("blink_eyes");
		haxe.Log.trace("CHIBI CLICK!!",{ fileName : "Main.hx", lineNumber : 90, className : "Main", methodName : "onChibiClickFN"});
	}
	,makeBody: function() {
		var ba;
		var this1;
		this1 = new Array(2);
		ba = this1;
		var val = openfl.Assets.getBitmapData("img/BODY_01.PNG");
		ba[0] = val;
		var val1 = openfl.Assets.getBitmapData("img/BODY_02.PNG");
		ba[1] = val1;
		var op = new jmSubLibs.decoupledLIB.refactor.BMDollSprite();
	...
	,makeEyes: function() {
		...
		this1 = new Array(5);
		ba = this1;
		var val = this.gBM("img/IDLE_EYES_01.PNG");
		ba[0] = val;
		var val1 = this.gBM("img/IDLE_EYES_02.PNG");
		ba[1] = val1;
		var val2 = this.gBM("img/BLINK_EYES_01.PNG");
		ba[2] = val2;
		var val3 = this.gBM("img/BLINK_EYES_02.PNG");
		ba[3] = val3;
		var val4 = this.gBM("img/BLINK_EYES_03.PNG");
		ba[4] = val4;
		var op = new jmSubLibs.decoupledLIB.refactor.BMDollSprite();
		op.loadFrames(ba,true);
		return op;
	...
	,MakeMouth: function() {
		...
		this1 = new Array(3);
		ba = this1;
		var val = openfl.Assets.getBitmapData("img/MOUTH_01.PNG");
		ba[0] = val;
		var val1 = openfl.Assets.getBitmapData("img/MOUTH_02.PNG");
		ba[1] = val1;
		var val2 = openfl.Assets.getBitmapData("img/MOUTH_03.PNG");
		ba[2] = val2;
		var op = new jmSubLibs.decoupledLIB.refactor.BMDollSprite();
		op.loadFrames(ba,true);
		return op;
	...
	,gBM: function(inName) {
		var op = openfl.Assets.getBitmapData(inName);
		return op;
	}
	,added: function(e) {
...
var jmSubLibs = {};
jmSubLibs.decoupledLIB = {};
jmSubLibs.decoupledLIB.consts = {};
jmSubLibs.decoupledLIB.consts.openfl = {};
jmSubLibs.decoupledLIB.consts.openfl.geom = {};
jmSubLibs.decoupledLIB.consts.openfl.geom.PointZZ = function() {
};
$hxClasses["jmSubLibs.decoupledLIB.consts.openfl.geom.PointZZ"] = jmSubLibs.decoupledLIB.consts.openfl.geom.PointZZ;
jmSubLibs.decoupledLIB.consts.openfl.geom.PointZZ.__name__ = ["jmSubLibs","decoupledLIB","consts","openfl","geom","PointZZ"];
jmSubLibs.decoupledLIB.consts.openfl.geom.PointZZ.prototype = {
	__class__: jmSubLibs.decoupledLIB.consts.openfl.geom.PointZZ
};
jmSubLibs.decoupledLIB.doll = {};
jmSubLibs.decoupledLIB.doll.ChibiSpace = function(inPixelWid,inPixelHig) {
	this._holdCount = 0;
	this.heldFrames = 1;
	this._u = 0;
	this._stackLen = 0;
	...
	this1 = new Array(0);
	this._stack = this1;
	this._buf = new openfl.display.BitmapData(inPixelWid,inPixelHig,true,0);
	this._dsp = new openfl.display.Bitmap();
	this._dsp.bitmapData = this._buf;
	this.addChild(this._dsp);
	this.addEventListener(openfl.events.Event.ENTER_FRAME,$bind(this,this.onEnterFrameUpdate));
	this.addEventListener(openfl.events.MouseEvent.CLICK,$bind(this,this.onClick));
};
$hxClasses["jmSubLibs.decoupledLIB.doll.ChibiSpace"] = jmSubLibs.decoupledLIB.doll.ChibiSpace;
jmSubLibs.decoupledLIB.doll.ChibiSpace.__name__ = ["jmSubLibs","decoupledLIB","doll","ChibiSpace"];
jmSubLibs.decoupledLIB.doll.ChibiSpace.__super__ = openfl.display.Sprite;
jmSubLibs.decoupledLIB.doll.ChibiSpace.prototype = $extend(openfl.display.Sprite.prototype,{
	add: function(inCMP) {
		var s2;
		var this1;
		this1 = new Array(this._stackLen + 1);
		s2 = this1;
		var i;
		var _g1 = 0;
		var _g = this._stackLen;
		while(_g1 < _g) {
			var i1 = _g1++;
			s2[i1] = this._stack[i1];
			this._stack[i1] = null;
		}
		this._stack = s2;
		this._stack[this._stackLen] = inCMP;
		this._stackLen = this._stack.length;
	}
	,update: function() {
		this._buf.fillRect(this._buf.rect,0);
		this._u = -1;
		while(true) {
			this._u++;
			if(this._u >= this._stackLen) break;
			this._cur = this._stack[this._u];
			this._cur.update();
			this._cur.renderTo(this._buf);
		}
	}
	,onEnterFrameUpdate: function(evt) {
		this._holdCount++;
	...
	,setClickFN: function(inFN) {
		this._clickFN = inFN;
		this._hasClickFN = true;
	}
	,onClick: function(evt) {
		haxe.Log.trace("TODO onClick",{ fileName : "ChibiSpace.hx", lineNumber : 147, className : "jmSubLibs.decoupledLIB.doll.ChibiSpace", methodName : "onClick"});
		if(this._hasClickFN) {
			haxe.Log.trace("calling click function.",{ fileName : "ChibiSpace.hx", lineNumber : 154, className : "jmSubLibs.decoupledLIB.doll.ChibiSpace", methodName : "onClick"});
			this._clickFN();
		}
	}
	,__class__: jmSubLibs.decoupledLIB.doll.ChibiSpace
});
jmSubLibs.decoupledLIB.doll.DollAnimData = function(inName,inAnim,inLoop) {
	this.loop = false;
	this.maxFrame = 0;
	this.curFrame = 0;
	this.name = inName;
	...
	this.loop = inLoop;
	this.curFrame = 0;
	this.maxFrame = inAnim.length - 1;
};
$hxClasses["jmSubLibs.decoupledLIB.doll.DollAnimData"] = jmSubLibs.decoupledLIB.doll.DollAnimData;
jmSubLibs.decoupledLIB.doll.DollAnimData.__name__ = ["jmSubLibs","decoupledLIB","doll","DollAnimData"];
jmSubLibs.decoupledLIB.doll.DollAnimData.prototype = {
	__class__: jmSubLibs.decoupledLIB.doll.DollAnimData
};
jmSubLibs.decoupledLIB.refactor = {};
jmSubLibs.decoupledLIB.refactor.BMDollSprite = function() {
	this._hasDefaultAnimName = false;
	this._hasActiveAnim = false;
	this._curFrame = 0;
	this._hasStrip = false;
	this._rec = new openfl.geom.Rectangle();
	this._dir = new jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint();
	this._pt = new openfl.geom.Point();
	this._anims = new Array();
};
$hxClasses["jmSubLibs.decoupledLIB.refactor.BMDollSprite"] = jmSubLibs.decoupledLIB.refactor.BMDollSprite;
jmSubLibs.decoupledLIB.refactor.BMDollSprite.__name__ = ["jmSubLibs","decoupledLIB","refactor","BMDollSprite"];
jmSubLibs.decoupledLIB.refactor.BMDollSprite.prototype = {
	setDefaultAnim: function(inName) {
		this._defaultAnimName = inName;
		this._hasDefaultAnimName = true;
	}
	,addAnim: function(inName,inFrames,inLoop) {
		var tmpAnimDat = new jmSubLibs.decoupledLIB.doll.DollAnimData(inName,inFrames,inLoop);
		this._anims.push(tmpAnimDat);
	}
	,getAnimIndex: function(inName) {
		var tmpAnim;
		var _g1 = 0;
		var _g = this._anims.length;
		while(_g1 < _g) {
			var i = _g1++;
			tmpAnim = this._anims[i];
			if(tmpAnim.name == inName) return i;
		}
		return -1;
	}
	,playAnim: function(inName) {
		var tmpAnimIndex = this.getAnimIndex(inName);
		if(tmpAnimIndex >= 0) {
			this._activeAnim = this._anims[tmpAnimIndex];
			this._hasActiveAnim = true;
			this._activeAnim.curFrame = 0;
		} else {
			haxe.Log.trace("anim not found!",{ fileName : "BMDollSprite.hx", lineNumber : 157, className : "jmSubLibs.decoupledLIB.refactor.BMDollSprite", methodName : "playAnim"});
	...
	,update: function() {
		if(this._hasActiveAnim) {
			...
			if(this._activeAnim.curFrame > this._activeAnim.maxFrame) {
				if(this._activeAnim.loop || false == this._hasDefaultAnimName) this._activeAnim.curFrame = 0; else {
					var tmpDex = this.getAnimIndex(this._defaultAnimName);
					if(tmpDex >= 0) {
						this._activeAnim = this._anims[tmpDex];
						this._activeAnim.curFrame = 0;
					} else haxe.Log.trace("Error: ANIM NOT FOUND!",{ fileName : "BMDollSprite.hx", lineNumber : 192, className : "jmSubLibs.decoupledLIB.refactor.BMDollSprite", methodName : "update"});
				}
			}
			this._curFrame = this._activeAnim.anim[this._activeAnim.curFrame];
		} else {
			this._curFrame++;
			if(this._curFrame > this._maxFrameIndex) this._curFrame = 0;
		}
	}
	,renderTo: function(inDrawOnBM) {
		if(false == this._hasStrip) return;
		this._pt.x = this.ix;
		this._pt.y = this.iy;
	...
	,loadFrames: function(inFrames,inDisposeWhenDoneLoading) {
		var i;
		if(inFrames == null) {
			throw "your frames are null!";
			return;
		}
		if(inFrames.length <= 0) {
			throw "you have no frames in here!";
			return;
		}
		this._numFrames = inFrames.length;
		this._maxFrameIndex = this._numFrames - 1;
		var bm;
		this._wid = inFrames[0].width;
		this._hig = inFrames[0].height;
		var _g1 = 0;
		var _g = this._numFrames;
		while(_g1 < _g) {
			var i1 = _g1++;
			bm = inFrames[i1];
			if(bm.width != this._wid) haxe.Log.trace("ERROR!!!! Frame wid not uniform amongst frames.",{ fileName : "BMDollSprite.hx", lineNumber : 269, className : "jmSubLibs.decoupledLIB.refactor.BMDollSprite", methodName : "loadFrames"}); else if(bm.height != this._hig) haxe.Log.trace("ERROR!!!! Frame wid not uniform amongst frames.",{ fileName : "BMDollSprite.hx", lineNumber : 274, className : "jmSubLibs.decoupledLIB.refactor.BMDollSprite", methodName : "loadFrames"});
		}
		this._dir.ix = 0;
		this._dir.iy = 1;
		var copyRec = new openfl.geom.Rectangle();
		copyRec.width = this._wid;
		copyRec.height = this._hig;
		copyRec.x = 0;
		copyRec.y = 0;
		var stripWid = this._wid;
		var stripHig = this._hig * this._numFrames;
		this._strip = new openfl.display.BitmapData(stripWid,stripHig,true,-65536);
		var b;
		var _g11 = 0;
		var _g2 = this._numFrames;
		while(_g11 < _g2) {
			var b1 = _g11++;
			bm = inFrames[b1];
			...
			if(inDisposeWhenDoneLoading) bm.dispose();
		}
		this._rec.width = this._wid;
		this._rec.height = this._hig;
		this._hasStrip = true;
	}
	,loadStrip: function(inStrip,inNumberOfFrames) {
		this._numFrames = inNumberOfFrames;
		this._maxFrameIndex = inNumberOfFrames - 1;
		var frameAllocationErrorCheck = 0;
		var stripWid = inStrip.width;
		var stripHig = inStrip.height;
		if(stripWid < stripHig) {
			this._dir.ix = 0;
			this._dir.iy = 1;
			this._wid = stripWid;
			this._hig = Math.floor(stripHig / this._numFrames);
			frameAllocationErrorCheck = this._hig * this._numFrames;
			if(frameAllocationErrorCheck != stripHig) haxe.Log.trace("ERROR::doesn't go in evenly! VER / HIG",{ fileName : "BMDollSprite.hx", lineNumber : 351, className : "jmSubLibs.decoupledLIB.refactor.BMDollSprite", methodName : "loadStrip"});
		} else if(stripHig < stripWid) {
			this._dir.ix = 1;
			this._dir.iy = 0;
			this._wid = Math.floor(stripWid / this._numFrames);
			this._hig = stripHig;
			frameAllocationErrorCheck = this._wid * this._numFrames;
			if(frameAllocationErrorCheck != stripWid) haxe.Log.trace("ERROR::doesn't go in evenly! HOR / WID",{ fileName : "BMDollSprite.hx", lineNumber : 366, className : "jmSubLibs.decoupledLIB.refactor.BMDollSprite", methodName : "loadStrip"});
		} else {
			this._wid = stripWid;
			this._hig = stripHig;
			this._dir.ix = 0;
	...
	,__class__: jmSubLibs.decoupledLIB.refactor.BMDollSprite
};
jmSubLibs.freeBinaryLIB = {};
jmSubLibs.freeBinaryLIB.structs = {};
jmSubLibs.freeBinaryLIB.structs.geom = {};
jmSubLibs.freeBinaryLIB.structs.geom.primitives = {};
jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint = function() {
};
$hxClasses["jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint"] = jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint;
jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint.__name__ = ["jmSubLibs","freeBinaryLIB","structs","geom","primitives","IntPoint"];
jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint.make = function(inX,inY) {
	var newPt = new jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint();
	newPt.ix = inX;
	newPt.iy = inY;
	return newPt;
};
jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint.addTwo = function(ptA,ptB,out) {
	if(out == null) out = new jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint();
	out.ix = ptA.ix + ptB.ix;
	out.iy = ptA.iy + ptB.iy;
	return out;
};
jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint.makeCopy = function(ip) {
	var out = new jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint();
	out.ix = ip.ix;
	out.iy = ip.iy;
	return out;
};
jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint.makeFrom1D = function(inOneDimensionalIndexPosition,width,height) {
	var newPt = new jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint();
	newPt.iy = Math.floor(inOneDimensionalIndexPosition / width);
	newPt.ix = inOneDimensionalIndexPosition - newPt.iy * width;
	return newPt;
};
jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint.compare = function(ptA,ptB) {
	if(ptA.ix != ptB.ix) return false;
	if(ptA.iy != ptB.iy) return false;
	return true;
};
jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint.prototype = {
	__class__: jmSubLibs.freeBinaryLIB.structs.geom.primitives.IntPoint
};
var js = {};
js.Boot = function() { };
...
jmSubLibs.decoupledLIB.consts.openfl.geom.PointZZ.ZZ = new openfl.geom.Point(0,0);
jmSubLibs.decoupledLIB.refactor.BMDollSprite.ZZ = jmSubLibs.decoupledLIB.consts.openfl.geom.PointZZ.ZZ;
...
//# sourceMappingURL=comChibiDoll01.js.map